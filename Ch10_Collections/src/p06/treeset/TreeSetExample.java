...
public class TreeSetExample {
	public static void main(String[] args) {
		...
		Integer score = null;
		score = scores.first();//정렬되어 저장되어있는 객체들 중 가장 작은 값의 객체
		System.out.println("가장 낮은 점수:"+score);
		score = scores.last();//정렬되어 저장되어있는 객체들 중 가장 큰 값의 객체
		System.out.println("가장 높은 점수:"+score);
		score = scores.lower(95);//해당객체바로 앞의 객체
		...
		while(!scores.isEmpty()) {//자료가 비어있는지 여부 확인 isEmpty()
			score =scores.pollFirst();//가장 작은 값을 추출하는 메소드
			System.out.println(score+"(남은 객쳇 수:"+scores.size()+")");
		}
