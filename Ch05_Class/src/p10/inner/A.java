...
public class A {//바깥쪽 클래스
	...
	//필드
		int field1;//인스턴스 필드
		//static int field2;//오류- 인스턴스 멤버클래스에는 static멤버필드가 올수 없음
	//메소드
		void method1() {}//인스턴스 메소드
		//static void method2() {}//오류-인스턴스멤버클래스에 static멤버메소드가 올수 없음.
	}
	//정적 멤버 클래스
	...
	void method1() {
		...
		class D{/* 외부에서는 로컬 클래스 D에 접근 할수 없음 */
			...
			//필드
			int field1;//인스턴스 필드
			//static int field2;// 오류-static필드는 올수없음
			//메소드
			void method(){}//인스턴스 메소드
			//static void method2() {}//오류 - static메소드는 올수 없음
			}//클래스 D끝.
	   //객체 생성
		D d = new D();//① 객체 생성
		d.field1 = 3; //② 로컬클래스의 멤버필드 초기화
		d.method();//② 로컬 메소드 를 메소드의 멤버화
	}//method1끝.
}//바깥 클래스 끝.
